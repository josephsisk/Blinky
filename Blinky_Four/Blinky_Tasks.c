...
//  uint32_t led_num = 0;
	...
	my_semaphore_id = osSemaphoreCreate(osSemaphore(my_semaphore), 2);  // Create semaphore with 2 tokens
...
void Thread_Blinky1 (void const *argument) {
  uint32_t led_num = 0;
	...
	int token_acquired = 0; //Used to indicate the semaphore
  ...
  while (1) {
		...
		token_acquired = osSemaphoreWait(my_semaphore_id, osWaitForever);			
		while(token_acquired == 0)
		{
